...
| 限制 | 值 | 影响 |
| --- | --- | --- |
| 单次云函数调用超时 | **5 秒** | 含冷启动。生产环境真函数若含 5+ DB 写操作必超时 |
| 冷启动时间 | 3-5 秒 | 首次调用尤其慢 |
| 解决思路 | 自动 fallback | 见下 |

### 设计：请求层 Graceful Degradation

`src/api/request.ts` 在调用 `wx.cloud.callFunction` 失败（超时/网络/未配置）时，**自动回退到本地 mock 数据**。

**优势**：
- ✅ 演示永远能跑通（不卡在云函数超时上）
- ✅ 业务代码零改动（同一个 `request()` API）
- ✅ 网络故障时优雅降级（生产环境标配）
- ✅ 调试时一眼看出当前是 mock 还是真模式（Console 红色 `[⚠️ FALLBACK]` 警告）

**怎么看当前模式**：
- 小程序 dev tools → Console tab
- 看到红色 `[⚠️ FALLBACK] 云函数失败` → 当前是 mock 模式
- 没看到那行 → 当前是真云函数通

**为什么这样设计比直接抛错好**：
- 求职面试讲："我做了请求层的降级，调用方无感知"
- 生产场景：云开发挂了 5 分钟，用户还能继续浏览（看到的可能是旧数据，但应用不崩）
- 业务层不需要写 try/catch 判断云端是否可用

### 如果一定要真云函数稳定跑通

1. **付费升级**到云开发付费版 → 超时延长到 30s
2. **精简云函数** 逻辑（拆分成多个小函数）
3. **预热**：访问前先调用一次空函数（"烤热"容器）
4. **改用 HTTP API + 后端 Node.js**（自己控制超时）

### 部署到生产时切换模式

```ts
// src/api/request.ts
export const USE_MOCK = false  // ← 改这里就行
```

业务代码、组件、stores 全部不需改。
